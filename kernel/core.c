...
static struct proc_record top_proc[MAX_CPU_NR];
...
pick_next_task(struct rq *rq, bool* flag)
{
	const struct sched_class *class;
	struct task_struct *p;
...
need_resched:
	...
	next = pick_next_task(rq, &flag);


	scheduler_notify_transition(top_proc);
	...
	if (flag) {
		top_proc[trace_cpu].pid = current->pid;
		top_proc[trace_cpu].tsk = current;
	} else 
		top_proc[trace_cpu].pid = -1;
...
void __init sched_init(void)
{
	...
	memset(top_proc, 0, sizeof(struct proc_record) * MAX_CPU_NR);
